...
- (id)initWithTexture:(Texture2D *)newtexture {
	return [self initWithTexture:newtexture origin:CGPointMake(0, 0) frame:CGRectZero stretch:CGRectZero buffer:nil];
}

- (id)initWithTexture:(Texture2D *)newtexture origin:(CGPoint)neworigin {
	return [self initWithTexture:newtexture origin:neworigin frame:CGRectZero stretch:CGRectZero buffer:nil];
}
	...
	if ((self = [super init])) {
		...
		if (!buffer) {
			if (stretch.size.width > 0 && stretch.size.height > 0 && (stretch.size.width < width || stretch.size.height < height)) {
				...
				buffer = [[Buffer2D alloc] initWithMode:GL_TRIANGLE_STRIP vertices:vertices vertexCount:sizeof(vertices) / sizeof(Buffer2DVertex) indices:indices indexCount:sizeof(indices) / sizeof(GLushort) isDynamic:NO];
			}
			else {
				//GLfloat vertices[] = {
